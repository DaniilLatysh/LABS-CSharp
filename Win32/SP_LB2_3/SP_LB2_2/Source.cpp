	...
	LPTSTR lpszCmdLine, int nCmdShow)
{
	...
	hSemaphore = CreateSemaphore(NULL, g_cMax, g_cMax, TEXT("Semaphore"));
...
void WndProc_OnCommand(HWND hWnd, int id, HWND hwndCtl, UINT codeNotify)
{
		...
		if (exitCodeThread != STILL_ACTIVE)
		{
			...
			/*if (g_Synhronization)
			{
				...
				while (dwWaitResult != WAIT_OBJECT_0)
				{
					...
					MessageBox(NULL, TEXT("Time out!"), TEXT(""), MB_OK);
				}
				Sleep(1000);
				if (ReleaseSemaphore(
	...
	case ID_THREAD1_DESTROY:
		...
		if (exitCodeThread == STILL_ACTIVE)
		{
			Suspend[1] = false;
			ResumeThread(hSecThread[1]);
			SetWindowLong(hWnd, GWL_USERDATA, 1L);
	...
	case ID_THREAD2_DESTROY:
		...
		if (exitCodeThread == STILL_ACTIVE)
		{
			Suspend[2] = false;
			ResumeThread(hSecThread[2]);
			SetWindowLong(hWnd, GWL_USERDATA, 2L);
	...
	case ID_SYNHRONIZATION_ON:
		g_Synhronization = true;
		break;

	case ID_SYNHRONIZATION_OFF:
		g_Synhronization = false;
		break;
...
DWORD WINAPI ThreadFunc1(PVOID pvParam)
{
	...
	MessageBox(NULL, TEXT("Thread Start!"), TEXT(""), MB_OK);
...
#define MAXCHAR 500
	...
	wsprintf(CreepingLine,
		TEXT("Вторичный поток создал Латыш Д.Н., поток - %d"),
	...
	while (TRUE)
	{
		exitFlag = GetWindowLong(ThrParam->hWnd, GWL_USERDATA);
	...
	{
		MessageBox(NULL, TEXT("Thread End!"), TEXT(""), MB_OK);
	}
